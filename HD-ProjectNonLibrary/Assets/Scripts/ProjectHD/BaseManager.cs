...
namespace ProjectHD
{
    public abstract class BaseManager<T> : Singleton<T> where T : MonoBehaviour
    {
        ...
        protected virtual async UniTask DeInitialize()
        {
            ExecuteDeInitializeStartEvent();
            await UniTask.DelayFrame(2);
        }

        protected void MoveToWorkspace(GameObject gameObject)
        {
            MainManager.Instance.MoveToCurrentWorkspace(gameObject);
        }

        protected void MoveToOherScene(ProjectEnum.SceneName sceneName, UniTask cleanUp)
        {
            SceneLoadManager.Instance.MoveToScene(sceneName, cleanUp);
        }

        protected void ExecuteInitialzeCompleteEvent(bool isDone)
        {
        ...
        protected void ExecuteDeInitializeStartEvent()
        {
            Event.EventManager.Broadcast(Event.Events.ManagerUnloadEvent);
        }

        protected void ExecuteSetActiveSceneEvent(UnityEngine.SceneManagement.Scene scene)
        {
